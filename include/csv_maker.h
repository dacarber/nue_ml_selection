...
void write_pair(const caf::SRSpillProxy* sr, const caf::SRInteractionTruthDLPProxy& i, const caf::SRInteractionDLPProxy& j)
{
    output  << CSV(sr->hdr.run) << CSV(sr->hdr.evt) << CSV(sr->hdr.subrun) << CSV(std::string(sr->hdr.sourceName))
            ...
            << CSV(vars::leading_electron_pid(i))
            << CSV(vars::leading_electron_pid(j))
            << CSV(vars::leading_proton_pid(i))
            << CSV(vars::leading_proton_pid(j))
            << CSV(vars::visible_energy(i))
            << CSV(vars::visible_energy(j))
            << CSV(vars::electron_polar_angle(i))
            << CSV(vars::electron_polar_angle(j))
            << CSV(vars::electron_azimuthal_angle(i))
            ...
            << CSV(vars::electron_softmax(j)) << CSV(vars::proton_softmax(j))
            << CSV(vars::non_primary_number(j))
            << CSV(vars::interaction_start_x(j))
            << CSV(vars::interaction_start_y(j))
            << CSV(vars::interaction_start_z(j))
            << CSV(vars::leading_electron_size(j))
            << CSV(vars::NuMI_nu_theta(i))
            << CSV(vars::NuMI_nu_theta(j))
            << CSV(vars::NuMI_nu_phi(i))
            << CSV(vars::NuMI_nu_phi(j))
            << CSV(vars::delta_pt(i))
            << CSV(vars::delta_alphaT(i))
            << CSV(vars::delta_phiT(i))
            << CSV(vars::delta_pt(j))
            << CSV(vars::delta_alphaT(j))
            << CSV(vars::delta_phiT(j))
            << CSV(vars::electron_transverse_momentum_mag(i))
            << CSV(vars::electron_transverse_momentum_mag(j))
            << CSV(vars::proton_transverse_momentum_mag(i))
            << CSV(vars::proton_transverse_momentum_mag(j))
